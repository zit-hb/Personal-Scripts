...
# It supports installing all updates, security updates only, checking for available updates, and excluding specific
# packages. It provides options to reboot the system if necessary after updates, simulate updates (dry run), and log
...
class APTManager(AutoUpdateManager):
    def install_updates(self, security_only: bool = False):
        ...
        if self.exclude:
            logging.warning(
                "Excluding packages is not directly supported with apt-get upgrade."
            )
    ...
    def cleanup(self):
        ...
        """
        cmds = [["apt-get", "autoclean", "-y"], ["apt-get", "autoremove", "-y"]]
        for cmd in cmds:
            if self.use_sudo:
                cmd = ["sudo"] + cmd
...
class YUMManager(AutoUpdateManager):
    ...
    def cleanup(self):
        ...
        """
        pkg_manager = "dnf" if Path("/usr/bin/dnf").exists() else "yum"
        cmds = [[pkg_manager, "autoremove", "-y"], [pkg_manager, "clean", "all"]]
        for cmd in cmds:
            if self.use_sudo:
                cmd = ["sudo"] + cmd
            logging.info(f"Running {' '.join(cmd)}")
...
def setup_logging(
    ...
    if log_file:
        logging.basicConfig(
            filename=log_file, level=level, format="%(levelname)s: %(message)s"
        )
    else:
...
def main():
    ...
    if distro_id in ["ubuntu", "debian"]:
        ...
        manager = APTManager(
            dry_run=args.dry_run, exclude=exclude_list, use_sudo=use_sudo
        )

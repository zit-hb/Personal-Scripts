...
def extract_install_command(line: str) -> Optional[str]:
    ...
    if match:
        ...
        if (
            any(token in ("pip", "pip3", "python", "python3") for token in tokens)
            and "pip" in tokens
        ):
            return command
        elif (
            ...
            or command.startswith("pip3 ")
            or command.startswith("python -m pip")
            or command.startswith("python3 -m pip")
        ):
            return command
        else:
            return None
...
def parse_pip_install_command(
    ...
    while i < len(tokens) and (
        tokens[i] in skip_tokens or tokens[i : i + 3] == ["python", "-m", "pip"]
    ):
        ...
        if token.startswith("-"):
            ...
            if token in options_with_args:
                ...
                if i < len(tokens):
                    ...
                    logging.warning(
                        f"Option '{token}' expects an argument, but none was provided."
                    )
            i += 1
        else:
...
def get_latest_version(package_name: str) -> Optional[str]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logging.error(
                f"Failed to get version info for package '{package_name}' from PyPI."
            )
    except Exception as e:
...
def get_installed_version(package_name: str) -> Optional[str]:
    ...
    try:
        ...
        logging.error(
            f"Error getting installed version for package '{package_name}': {e}"
        )
        return None
    ...
    for spec in package_specifiers:
        ...
        if update_package:
            ...
            if version:
                ...
                updated_specifiers.append(spec)
                logging.warning(
                    f"Could not get version for package '{package_name}', leaving as is."
                )
        else:
            updated_specifiers.append(spec)
...
def process_script(script_path: str, args: argparse.Namespace):
    ...
    if requirements_start is None:
        logging.info(
            f"No 'Requirements' section found in script '{script_path}'. Skipping."
        )
        return
    ...
    new_header_lines = (
        header_lines[: requirements_start + 1]
        + updated_requirements_lines
        + header_lines[requirements_end:]
    )

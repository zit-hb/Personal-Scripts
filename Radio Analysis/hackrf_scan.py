...
class HackRFDevice:
    ...
    def set_baseband_filter_bandwidth(self, bw_hz: float) -> None:
        ...
        result = _libhackrf.hackrf_set_baseband_filter_bandwidth(self._dev, ctypes.c_uint32(valid_bw))
        if result != HACKRF_SUCCESS:
            raise RuntimeError(f"Error setting baseband filter BW to {valid_bw} Hz, code={result}")
...
class HackRFScanner:
        ...
        for cfreq in freq_plan:
            ...
            for _capture_idx in range(self.repeat_captures):
                ...
                if len(cplx_iq) < self.fft_size:
                    ...
                    cplx_iq = np.concatenate([cplx_iq, np.zeros(pad_len, dtype=np.complex64)])
                else:
                ...
                noise_lin = compute_noise_floor_linear(power_lin, robust=self.robust_floor)
    ...
    def _find_stable_bins(
        self,
        captures_bins: List[List[Tuple[float, float, float]]]
    ) -> List[Tuple[float, float, float]]:
        """
    ...
    def _create_freq_steps_for_range(self, rng_start: float, rng_end: float) -> List[float]:
        """
    ...
    def merge_final_ranges(
        ...
        temp = [(r.start_hz, r.end_hz, r.peak_power_db, r.likely_protocol) for r in ranges]
        ...
        final_class = HackRFScanner._compute_merged_classification(cur_block, cur_start, cur_end)
        merged.append(
            ActiveRange(
                start_hz=cur_start,
                end_hz=cur_end,
...
def main() -> None:
    ...
    merged_ranges = scanner.merge_final_ranges(scanner.active_ranges, scanner.merge_gap_khz)

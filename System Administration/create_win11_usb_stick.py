...
def create_windows_usb(iso_path: str, device: str) -> None:
    ...
    try:
        ...
        # Ensure kernel knows about the new partition table
        subprocess.run(["partprobe", device], check=True)

        # If the OS automatically mounts the partitions, unmount them now.
        logging.info("Attempting to unmount partitions if automounted.")
        for part_num in [1, 2]:
            auto_part = f"{device}{part_num}"
            subprocess.run(["umount", auto_part], check=False)
